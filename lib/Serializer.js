...
const serializeMessage = async (client, msg) => {
  ...
  if (msg.message) {
    ...
    try {
      msg.quoted = client?.store ? await serializeMessage(client, (await client.store.loadMessage(
        msg.isGroup ? msg.from : msg.sender,
        msg.content?.contextInfo?.stanzaId
      ))) : null;
    } finally {
      if (!msg.quoted && q) {
        msg.quoted = await serializeMessage(client, proto.WebMessageInfo.fromObject({
          key: {
