...
module.exports = async function (client, msg) {
  ...
  let header_sender =
    chalk[pickRandom(colors)](
      "~ " +
        (msg.key.fromMe ? client.user.name : msg.pushName) +
        " " +
        PhoneNumber("+" + (msg.key.fromMe ? serializeJid(client.user.id).split("@")[0] : msg.author.split("@")[0])).getNumber("international")
    ) +
    " to " +
    chalk.green(
      msg.from +
        ...
        (msg.isGroup
          ...
          : areJidsSameUser(serializeJid(client.user.id), msg.author)
          ? client.user.name
          : msg.pushName)
    ) +
    " " +
