...
module.exports = async function (bot, msg, groupMetadata) {
  ...
  const senderJid = msg.sender || msg.key.remoteJid;
  const timestamp = new Date(1000 * (msg.messageTimestamp?.low || msg.messageTimestamp || Date.now() / 1000)).toTimeString();

  // --- PERBAIKAN UTAMA: HANDLE groupMetadata YANG NULL ---
  ...
  let header_bot = chalk.red("~ " + (bot.user.name || 'Bot') + " " + formatPhoneNumber(botJid)) + " " + chalk.black(chalk.bgYellow(timestamp));
  ...
  if (msg.msg?.contextInfo?.mentionedJid) {
    for (let user of msg.msg.contextInfo.mentionedJid) {
      // Coba cari nama, jika tidak ada, gunakan nomor
